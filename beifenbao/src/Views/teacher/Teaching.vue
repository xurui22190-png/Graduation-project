...
const openWeightDialog = (row) => {
  currentTeachingId.value = row.tcId || row.tcid
  // 初始化，处理小数与百分比转换
  weightForm.wExam = Number(((row.w_exam || 0) * 100).toFixed(0))
  weightForm.wRegular = Number(((row.w_regular || 0) * 100).toFixed(0))
  weightForm.wTest = Number(((row.w_test || 0) * 100).toFixed(0))
...
const handleSaveWeights = async () => {
  ...
  try {
    const response = await request.post('/teaching/updateWeights', {
      tcId: currentTeachingId.value,
      wExam: weightForm.wExam / 100,
      wRegular: weightForm.wRegular / 100,
      wTest: weightForm.wTest / 100
    })
    ...
    if (res.code === 200 || res._code === 200) {
      ...
      fetchTableData() // 刷新列表，展示最新权重
    } else {
      ElMessage.error(res.msg || res._msg || '保存失败')
    }
  } catch (error) {
    ElMessage.error('保存失败，请检查接口')
  } finally {
